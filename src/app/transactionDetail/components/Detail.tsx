...
import CCTPLogo from '@/app/transfer/components/Icon/CCTPLogo'

dayjs.extend(utc)
dayjs.extend(timezone)

type TransferUserInfoProps = {
  type?: 'out' | 'in' | 'text' | 'swap' | 'crossChain'
  data?: ExtraInfo
  price?: number
}
...
const TransferUserInfo = ({
  ...
  const map = {
    ...
    crossChain: { img: '/imgs/out.png', color: '#FFF' }
  }
...
export const SuccessCrossDetail = ({ className }: SuccessCrossDetailProps) => {
  ...
  return (
    <div className={className}>
      ...
      ></TransferDetailInfo>
      <TransferUserInfo
        type={transactionDetail?.extraInfo?.[0].type as any}
      ></TransferUserInfo>
      <TransferDetailInfo
