      ...
      <div className="text-[12px] flex flex-col ">
        {type === "text" && (
          ...
          <div className="max-w-[180px] text-right">
            charged <em className="text-[#FFF] not-italic font-bold">{Number(transactionDetail?.gasFee?.usdValue).toFixed(4)} USDT</em> for
            swap and transfer
          </div>
        )}
        {type !== "text" && type !== "swap"&& (
          ...
          <div>
            ...
            <span className="font-bold inline-block" style={{ color }}>{amount||Number(transactionDetail?.value).toFixed(4)} {tokenName ||transactionDetail?.tokenName}</span>
            {
                data?null:<div className="text-right"> 
            ${Number(transactionDetail?.amount).toFixed(2)}</div>
            }
...
export const SuccessCrossDetail = ({ className }: SuccessCrossDetailProps) => {
  ...
  return (
    <div className={className}>
      <TransferUserInfo data={transactionDetail?.extraInfo[0]}></TransferUserInfo>
      <TransferDetailInfo data={extraInfoData?.[0]}></TransferDetailInfo>
      <TransferUserInfo type="swap"></TransferUserInfo>
      <TransferDetailInfo data={extraInfoData?.[1]}></TransferDetailInfo>
      <TransferUserInfo type="in" data={transactionDetail?.extraInfo[1]}></TransferUserInfo>
    </div>
  );
};
